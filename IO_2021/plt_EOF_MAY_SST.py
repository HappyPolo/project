...
'''
Author       : ChenKt
Date         : 2021-08-25 20:47:01
LastEditors  : ChenKt
LastEditTime : 2021-08-26 17:25:33
FilePath     : /project/IO_2021/plt_EOF_MAY_SST.py
Aim          : plot IO SST EOF results
Mission      :
'''
# %%
# %%
...
def plt_eofs(lp, le, frac, title):
  ...
  ax1.set_title(
      title+' MAY EOF1 (%s' % (round(frac[0], 2))+'%)', fontdict=font, loc='left')
  ...
  p2 = ax3.contourf(
      ...
      alpha=0.8, levels=np.arange(-1., 1.01, 0.1), vmin=-1., vmax=1., zorder=0)
  ax3.set_title(
      title+' MAY EOF2 (%s' % (round(frac[1], 2))+"%)", fontdict=font, loc='left')
  ...
  p3 = ax5.contourf(
      ...
      alpha=0.8, levels=np.arange(-1., 1.01, 0.1), vmin=-1., vmax=1., zorder=0)
  ax5.set_title(
      title+' MAY EOF3 (%s' % (round(frac[2], 2))+'%)', fontdict=font, loc='left')
  ...
  savedir = '../../plot/IO_2021/EOF/MAY/EOF_MAY_SST_'+title+'.jpg'
  if os.path.isfile(savedir):
    os.remove(savedir)
  # plt.subplots_adjust(wspace=-0.2, hspace=0.3)
  plt.savefig(savedir, dpi=300, format='jpg',
...
for i in range(6):
  ...
  dir = path+'EOF/EOF_MAY_'+titles[i]+'_pickle.dat'
