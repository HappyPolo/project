...
'''
Author       : ChenKt
Date         : 2021-08-25 11:49:21
LastEditors  : ChenKt
LastEditTime : 2021-08-25 11:51:37
FilePath     : /kt/project/IO_2021/MCA_MAY_SST_PRC_UV.py
Aim          : 对MAY IO(BIO/NIO/SIO) SST 与 降水/风场进行MCA，并保存数据
Mission      : 
...
# %%
# note : MAY
sst_MAY = standardize(detrend_dim(
    lsmask(selMon(SST, 5), path+fls), 'time'))
prc_MAY = standardize(detrend_dim(selMon(PRC, 5), 'time'))
u_MAY = standardize(detrend_dim(selMon(U, 5), 'time'))
v_MAY = standardize(detrend_dim(selMon(V, 5), 'time'))
sst_MAY['time'] = prc_MAY['time']
# %%
# note : BIO
I = 0
L = sst_MAY.sel(lat=slice(LlatS[I], LlatN[I]), lon=slice(LlonL, LlonR))

Rp = prc_MAY.sel(lat=slice(RlatS, RlatN), lon=slice(RlonL, RlonR))
Ru = u_MAY.sel(lat=slice(RlatS, RlatN), lon=slice(RlonL, RlonR), level=850)
Rv = v_MAY.sel(lat=slice(RlatS, RlatN), lon=slice(RlonL, RlonR), level=850)
x = Rp.time
# %%
L['time'] = Rp['time']
# %%
...
with open(path+'MCA/MCA_MAY_BIO_pickle.dat', 'wb') as f:
...
v_out = [vle, vre, vlt, vrt, vlphet, Fracv, rphetv, Corrv]
svd_out = [s_out, u_out, v_out]
with open(path+'MCA/MCA_MAY_NIO_pickle.dat', 'wb') as f:
    pkl = pickle.dump([s_out, u_out, v_out], f, protocol=-1)
...
v_out = [vle, vre, vlt, vrt, vlphet, Fracv, rphetv, Corrv]
svd_out = [s_out, u_out, v_out]
with open(path+'MCA/MCA_MAY_TIO_pickle.dat', 'wb') as f:
    pkl = pickle.dump([s_out, u_out, v_out], f, protocol=-1)
...
v_out = [vle, vre, vlt, vrt, vlphet, Fracv, rphetv, Corrv]
svd_out = [s_out, u_out, v_out]
with open(path+'MCA/MCA_MAY_SIO1_pickle.dat', 'wb') as f:
    pkl = pickle.dump([s_out, u_out, v_out], f, protocol=-1)
...
v_out = [vle, vre, vlt, vrt, vlphet, Fracv, rphetv, Corrv]
svd_out = [s_out, u_out, v_out]
with open(path+'MCA/MCA_MAY_SIO2_pickle.dat', 'wb') as f:
    pkl = pickle.dump([s_out, u_out, v_out], f, protocol=-1)
...
v_out = [vle, vre, vlt, vrt, vlphet, Fracv, rphetv, Corrv]
svd_out = [s_out, u_out, v_out]
with open(path+'MCA/MCA_MAY_SIO3_pickle.dat', 'wb') as f:
    pkl = pickle.dump([s_out, u_out, v_out], f, protocol=-1)
