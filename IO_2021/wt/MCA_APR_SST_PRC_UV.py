'''
Author       : ChenKt
Date         : 2021-08-28 19:37:04
LastEditors  : ChenKt
LastEditTime : 2021-08-28 19:54:33
FilePath     : /project/IO_2021/MCA_APR_SST_PRC_UV.py
Aim          : 
Mission      : 
'''
# %%
...
# %%
# note : APR
sst_APR = standardize(detrend_dim(
    lsmask(selMon(SST, 4), path+fls), 'time'))
prc_APR = standardize(detrend_dim(selMon(PRC, 4), 'time'))
u_APR = standardize(detrend_dim(selMon(U, 4), 'time'))
v_APR = standardize(detrend_dim(selMon(V, 4), 'time'))
sst_APR['time'] = prc_APR['time']
# %%
# note : BIO
I = 0
L = sst_APR.sel(lat=slice(LlatS[I], LlatN[I]), lon=slice(LlonL, LlonR))

Rp = prc_APR.sel(lat=slice(RlatS, RlatN), lon=slice(RlonL, RlonR))
Ru = u_APR.sel(lat=slice(RlatS, RlatN), lon=slice(RlonL, RlonR), level=850)
Rv = v_APR.sel(lat=slice(RlatS, RlatN), lon=slice(RlonL, RlonR), level=850)
x = Rp.time
# %%
L['time'] = Rp['time']
# %%
...
with open(path+'MCA/MCA_APR_BIO_pickle.dat', 'wb') as f:
...
v_out = [vle, vre, vlt, vrt, Fracv, rphetv, Corrv]
svd_out = [s_out, u_out, v_out]
with open(path+'MCA/MCA_APR_NIO_pickle.dat', 'wb') as f:
  pkl = pickle.dump([s_out, u_out, v_out], f, protocol=-1)
...
v_out = [vle, vre, vlt, vrt, Fracv, rphetv, Corrv]
svd_out = [s_out, u_out, v_out]
with open(path+'MCA/MCA_APR_TIO_pickle.dat', 'wb') as f:
  pkl = pickle.dump([s_out, u_out, v_out], f, protocol=-1)
...
v_out = [vle, vre, vlt, vrt, Fracv, rphetv, Corrv]
svd_out = [s_out, u_out, v_out]
with open(path+'MCA/MCA_APR_SIO1_pickle.dat', 'wb') as f:
  pkl = pickle.dump([s_out, u_out, v_out], f, protocol=-1)
...
v_out = [vle, vre, vlt, vrt, Fracv, rphetv, Corrv]
svd_out = [s_out, u_out, v_out]
with open(path+'MCA/MCA_APR_SIO2_pickle.dat', 'wb') as f:
  pkl = pickle.dump([s_out, u_out, v_out], f, protocol=-1)
...
v_out = [vle, vre, vlt, vrt, Fracv, rphetv, Corrv]
svd_out = [s_out, u_out, v_out]
with open(path+'MCA/MCA_APR_SIO3_pickle.dat', 'wb') as f:
  pkl = pickle.dump([s_out, u_out, v_out], f, protocol=-1)
