'''
Author       : ChenKt
Date         : 2021-08-28 19:39:42
LastEditors  : ChenKt
LastEditTime : 2021-08-29 19:41:58
FilePath     : /project/IO_2021/MCA_MAR_SST_PRC_UV.py
Aim          : 
Mission      : 
'''
# %%
...
# %%
# note : MAR
sst_MAR = standardize(detrend_dim(
    lsmask(selMon(SST, 3), path+fls), 'time'))
prc_MAR = standardize(detrend_dim(selMon(PRC, 3), 'time'))
u_MAR = standardize(detrend_dim(selMon(U, 3), 'time'))
v_MAR = standardize(detrend_dim(selMon(V, 3), 'time'))
sst_MAR['time'] = prc_MAR['time']
# %%
# note : BIO
I = 0
L = sst_MAR.sel(lat=slice(LlatS[I], LlatN[I]), lon=slice(LlonL, LlonR))

Rp = prc_MAR.sel(lat=slice(RlatS, RlatN), lon=slice(RlonL, RlonR))
Ru = u_MAR.sel(lat=slice(RlatS, RlatN), lon=slice(RlonL, RlonR), level=850)
Rv = v_MAR.sel(lat=slice(RlatS, RlatN), lon=slice(RlonL, RlonR), level=850)
x = Rp.time
# %%
L['time'] = Rp['time']
# %%
...
with open(path+'MCA/MCA_MAR_BIO_pickle.dat', 'wb') as f:
...
v_out = [vle, vre, vlt, vrt, Fracv, rphetv, Corrv]
svd_out = [s_out, u_out, v_out]
with open(path+'MCA/MCA_MAR_NIO_pickle.dat', 'wb') as f:
  pkl = pickle.dump([s_out, u_out, v_out], f, protocol=-1)
...
v_out = [vle, vre, vlt, vrt, Fracv, rphetv, Corrv]
svd_out = [s_out, u_out, v_out]
with open(path+'MCA/MCA_MAR_TIO_pickle.dat', 'wb') as f:
  pkl = pickle.dump([s_out, u_out, v_out], f, protocol=-1)
...
v_out = [vle, vre, vlt, vrt, Fracv, rphetv, Corrv]
svd_out = [s_out, u_out, v_out]
with open(path+'MCA/MCA_MAR_SIO1_pickle.dat', 'wb') as f:
  pkl = pickle.dump([s_out, u_out, v_out], f, protocol=-1)
...
v_out = [vle, vre, vlt, vrt, Fracv, rphetv, Corrv]
svd_out = [s_out, u_out, v_out]
with open(path+'MCA/MCA_MAR_SIO2_pickle.dat', 'wb') as f:
  pkl = pickle.dump([s_out, u_out, v_out], f, protocol=-1)
...
v_out = [vle, vre, vlt, vrt, Fracv, rphetv, Corrv]
svd_out = [s_out, u_out, v_out]
with open(path+'MCA/MCA_MAR_SIO3_pickle.dat', 'wb') as f:
  pkl = pickle.dump([s_out, u_out, v_out], f, protocol=-1)
