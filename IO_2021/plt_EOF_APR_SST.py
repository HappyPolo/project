'''
Author       : ChenKt
Date         : 2021-08-29 16:50:38
LastEditors  : ChenKt
LastEditTime : 2021-08-30 17:25:04
FilePath     : /project/IO_2021/plt_EOF_APR_SST.py
Aim          :
Mission      :
'''
# %%
...
def plt_eofs(lp, le, frac, title):
  ...
  ax1.set_title(
      title+' APR EOF1 (%s' % (round(frac[0], 2))+'%)', fontdict=font, loc='left')
  ...
  p2 = ax3.contourf(
      ...
      alpha=0.8, levels=np.arange(-1., 1.01, 0.1), vmin=-1., vmax=1., zorder=0)
  ax3.set_title(
      title+' APR EOF2 (%s' % (round(frac[1], 2))+"%)", fontdict=font, loc='left')
  ...
  p3 = ax5.contourf(
      ...
      alpha=0.8, levels=np.arange(-1., 1.01, 0.1), vmin=-1., vmax=1., zorder=0)
  ax5.set_title(
      title+' APR EOF3 (%s' % (round(frac[2], 2))+'%)', fontdict=font, loc='left')
  ...
  savedir = '../../plot/IO_2021/EOF/APR/EOF_APR_SST_'+title+'.jpg'
...
for i in range(6):
  ...
  dir = path+'EOF/EOF_APR_'+titles[i]+'_pickle.dat'
