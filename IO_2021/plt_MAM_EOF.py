...
FilePath     : /project/IO_2021/plt_EOF_MAM_SST.py
...
def plt_eofs(lp, le, frac, title):
  ...
  ax1.set_title(
      title+' MAM EOF1 (%s' % (round(frac[0], 2))+'%)', fontdict=font, loc='left')
  ...
  p2 = ax3.contourf(
      ...
      alpha=0.8, levels=np.arange(-1., 1.01, 0.1), vmin=-1., vmax=1., zorder=0)
  ax3.set_title(
      title+' MAM EOF2 (%s' % (round(frac[1], 2))+"%)", fontdict=font, loc='left')
  ...
  p3 = ax5.contourf(
      ...
      alpha=0.8, levels=np.arange(-1., 1.01, 0.1), vmin=-1., vmax=1., zorder=0)
  ax5.set_title(
      title+' MAM EOF3 (%s' % (round(frac[2], 2))+'%)', fontdict=font, loc='left')
  ...
  savedir = '../../plot/IO_2021/EOF/MAM/EOF_MAM_SST_'+title+'.jpg'
...
for i in range(6):
  ...
  dir = path+'EOF/EOF_MAM_'+titles[i]+'_pickle.dat'
